...
export async function runPreparedReply(
  ...
  const dmHistoryBody = ctx.DmHistoryBody?.trim();
  const dmContextNote = dmHistoryBody ? `[DM history - for context]\n${dmHistoryBody}` : undefined;
  ...
  const prefixedBody = [dmContextNote, threadContextNote, prefixedBodyBase]
    .filter(Boolean)
    .join("\n\n");
